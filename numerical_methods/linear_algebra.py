...
def kernel_basis(A: list[list[float]]) -> np.ndarray:
    ...
    param_cols_index = [j for j in range(m) if j not in pivot_cols_index]
    basis = []

    print(pivot_cols_index)
    print(pivot_rows_index)
    print(param_cols_index)
